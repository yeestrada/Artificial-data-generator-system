...
def rows_to_columns(data):
    ...
    for line in items:
        ...
        _type = 'integer' if len(line) - is_float > is_float else 'float'
        data_type.append(_type)
        round.append(stats.mode(
            [len(j[j.find('.') + 1:])
                for j in [i for i in line if '.' in i]]) if is_float else 0)
...
def save_data(file_name, data_to_save, data_type, round_to, prev_data=None, min_col=1):
    try:
        for i in range(len(data_type)):
            data_to_save[i] = np.around(data_to_save[i],
                                        decimals=0 if data_type[i] == 'integer' else round_to[i])
...
def selectAIC(values, difference_umbral=0.01, test='ad', check_test='aic'):
    try:
        ...
        for v in values:
            ...
            ad, ks, cvm, aic, bic = v[0]['ad'], v[0]['ks'], v[0]['cvm'], \
                                    v[0]['aic'], v[0]['bic']
...
def getDistributionInfo(fixed_data, data_type, difference_umbral, prueba, criterio):
    ...
    for i in range(len(fixed_data)):
        vector = R.FloatVector(fixed_data[i]) if data_type[i] == 'float' else R.IntVector(
            fixed_data[i])
        values = []
...
# @param prueba: prueba que se debe usar (ad,ks,cvm)
# @param check_test: criterio de a utilizar (aic, bic)
def generateData(fixed_data, data_type, fdata, difference_umbral, prueba, criterio):
    generated_data = []
    ...
    for i in range(len(fixed_data)):
        ...
        response = get_random_data_r(selected, len(fdata[fdata['max']]) - len(fdata[fdata['min']]),
                                     repeated)
